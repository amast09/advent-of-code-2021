module Lib (calculateAnswers, fileToInts, fileToCommands, fileToStrings) where
...
fileToStrings :: String -> IO (Maybe [String])
fileToStrings filePath = do
  maybeFileContents <- safeLoadFile' filePath
  return $ fmap lines maybeFileContents
...
fileToInts filePath = do
  maybeLines <- fileToStrings filePath
